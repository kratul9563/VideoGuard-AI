...
## documents loaders and splitter
...
def index_docs():
    '''
    Read the PDFs from backend/data , chunks them and upload them to Azure AI Search
    '''
    ## define path, we look for data folders 
    current_dir = os.path.dirname(os.path.abspath(__file__))
    data_folder = os.path.join(current_dir, "../../backend/data")


    ## check the enviornment variables
    logger.info("=" * 60)
    ...
    # VALIDATE THE REQUIRED ENVIORNMENT VARIABLES
    required_vars = [
    ...
    if missing_vars:
        logger.error(f"Missing required enviornment variable : {missing_vars}")
    ...
    # This turns into numbers (vectors)
    # Must matxh the model you deployed in Azure AI Foundry ('text-embedding-3-small')
    try:
        ...
        embeddings = AzureOpenAIEmbeddings(
            ...
            open_api_version = os.getenv("AZURE_OPENAI_API_VERSION", "2025-01-01"),
        )
    ...
    for pdf_path in pdf_files:
        try:
            ...
            ##9.  chunking strategy
            ## we split text into 1000 character chunks with character overlap
            ## to ensure context is not lost between cuts.
